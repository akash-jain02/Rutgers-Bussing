...
4. Tap **Set up my trip**. Choose a route and stop, answer “On average, how many minutes does it take you to walk to this bus stop?”, then set an optional buffer (0 means none). Walking time is required, not silently assumed.
5. Tap **Save**. The main screen shows the recommendation, leave-by time, next two predictions, walk/buffer, alerts, and source update time.
6. Use the demo scenario picker to inspect normal, too-close, delayed, missing, stale, and final-bus cases. **Demo data is simulated and must not be used for travel.**
...
In the iPhone app, open **Data source**, turn off **Use demo data**, set `http://localhost:8000` for Simulator, and tap **Apply**. Then select a real route and stop in **Edit trip**. Changing data sources clears the saved trip to avoid mixing demo IDs with live IDs.
